...
def constru_pai_a(tag_lines,Y):
    ...
    #print(tag_matri)    
    pai = np.array([ 1.0 for i in range(len(tag_lines))])
    a = np.array([[ 1.0 for i in range(len(tag_lines))]for j in range(len(tag_lines))])
    for row in Y:
...
def constru_b(tag_lines,word_lines,X,Y):
    ...
    b = np.array([[1.0 for i in range(len(word_lines))] for j in range(len(tag_lines))])
...
if __name__== "__main__":
    ...
    f_index_tag = open(index_tag,'r')
    f_index_word = open(index_word,'r')
    f_train = open(tr_input,'r')
    ...
    index = 0
    for line in pai:
        f_hmm_prior.write(str(format(line,'.20e')))
        index = index + 1
        if index != len(pai):
            f_hmm_prior.write('\n')    
    ...
    for line in b:     
        for w in range(len(line)):
            if w != len(line) - 1:
                f_hmm_emit.write(str(format(line[w],'.20e')) + ' ')
            else:
                f_hmm_emit.write(str(format(line[w],'.20e')))
        index = index + 1
        if index != len(a):        
    ...
    for line in a:
        for w in range(len(line)):
            if w != len(line) - 1:
                f_hmm_tran.write(str(format(line[w],'.20e')) + ' ')
            else:
                f_hmm_tran.write(str(format(line[w],'.20e')))
        index = index + 1
        if index != len(b):   
